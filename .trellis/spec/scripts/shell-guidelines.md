...
### Severity follows whether the fix is available

`doctor.sh` reports a missing **Trellis surface** (the `.pi/` adapters,
`.agents/skills/trellis-*`) as `bad` when the `trellis` CLI is on `PATH`, and as
`warn` when it is not:

> **Bad when the fix is available and was not applied; warn when the fix is not
> available.**

Those surfaces are gitignored and regenerated by `scripts/install-trellis.sh`,
which needs the CLI. On a machine that has the CLI, a missing surface is drift a
`./setup.sh` would repair — a defect. Without the CLI it is an environment choice:
`install-trellis.sh` prints the matching `note` and exits `0` so the Pi harness
still installs.

The same rule is why two checks that used to `warn` now `bad`:
`.pi/agents/*.md` and `.pi/extensions/trellis/index.ts`. The second matters most,
because the `SHIPPED_TOOLS` comparison reads that file — a `warn` there let the
section keep printing while it had stopped being able to see anything, which is
the failure the check-integrity rule under [Anti-Patterns](#anti-patterns)
forbids. The branch is computed once, near the top of `doctor.sh`, so both sites
and the `==> Trellis adapters` section cannot disagree about it.
