...
| Child-inert marker | `pi-agent/extensions/trellis-subagents-bridge/index.ts` (`CHILD_INERT_MARKER` — **the single definition**), `scripts/doctor.sh` (reads that declaration and checks the generated extension still returns on the marker *ahead of* its first `pi.registerTool?.(` call), and the generated `.pi/extensions/trellis/index.ts` carrying that guard — which this repo **cannot edit** | a dispatched child resumes the *parent's* per-turn session path and is handed a planning breadcrumb while its own definition tells it to review: `trellis update` renames the variable or moves the guard and nothing notices. `doctor.sh` fails instead |
| Bridge role markers (`CHILD_INERT_MARKER`, `BRIDGE_CHILD_MARKER`, `DISPATCH_AGENT_ENV`) | `pi-agent/extensions/trellis-subagents-bridge/index.ts` (each `const`, plus the `process.env[<name>] =` line that sets it), `scripts/doctor.sh` (fails when a declared `*_MARKER` / `*_ENV` constant has no assignment) | a marker the role predicates read with nothing setting it makes that role **unreachable**, and it fails silently: measured 2026-09-15, the child branch returned at extension load and every dispatched child received no context at all while `doctor.sh` stayed green — the extension loaded and the child reported `no_task`. Note the boundary: names the bridge *inherits* (`PI_SUBAGENT_CHILD`, `TRELLIS_CONTEXT_ID`) are read-only here and are deliberately **not** sites of this fact |
| `context_injection` limits in `.trellis/config.yaml` | `.trellis/config.yaml` (**the single definition**), the generated `.pi/extensions/trellis/index.ts` (`readContextInjectionLimits` — cannot be edited here), `.trellis/scripts/` (`task.py validate`, which warns against `max_file_bytes`), and `pi-agent/extensions/trellis-subagents-bridge/index.ts` (the injected child block) | a change applied to two readers out of three: the validator warns about a file the bridge injects in full, or the bridge truncates at a limit the config no longer sets. The bridge and the extension both **read** this section, with their own copies of the same fallback numbers (`32768` / `65536` / `131072`) — that duplication of the *fallback* is deliberate and must stay inert: a reader that restated a live value would be a third definition |
| Trellis surfaces (`.pi/` adapters, `.agents/skills/trellis-*`, `.trellis/.developer`, `.trellis/.template-hashes.json`) | `scripts/install-trellis.sh` (the single definition of the completeness test and the three role-agent names), `scripts/doctor.sh` (the same role list, plus the conditional severity for a missing surface), `setup.sh` (step 7, its header comment, and `--skip-trellis`), `README.md` (setup step list, flag list, Layout table, the `## Trellis surfaces` section, the day-to-day row, the script table), `.trellis/spec/scripts/index.md` (bash runtime table, `100755` list, the Node-preflight exception), `.trellis/spec/scripts/shell-guidelines.md` (the `prune` verb and the availability rule), `.trellis/spec/config/layout-and-surfaces.md` (the `.pi/`/`.agents/` row and the two `.trellis/` surfaces), `.trellis/spec/index.md` (the bash-script count) | a fresh clone silently loses the Trellis extension, the `/trellis-*` prompt commands, and the three role agents `pi-subagents` discovers — and `trellis update` cannot restore them while reporting `Already up to date!` |
